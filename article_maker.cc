...
#ifdef USE_QTWEBKIT
	// Evaluate webkit_blocking.js now to call
	// gdArticleView.onJsPageInitStarted() ASAP.
...
#endif
	// Start reading the deferred scripts early so that they are ready when
	// needed.
	"<script defer src='qrc:///scripts/deferred.js'></script>"
#ifndef USE_QTWEBKIT
	// Load webengine_deferred.js in the end because it calls
...
class CssAppender {
   public:
    /// @param needPageBackgroundColor_ whether findPageBackgroundColor() will
    /// be called.
    explicit CssAppender(string &result_, bool needPageBackgroundColor_)
	: result(result_),
	  needPageBackgroundColor(needPageBackgroundColor_),
	  isPrintMedia(false) {}

    /// Style sheets appended after a call to this function apply only while
    /// printing.
    /// @note: style sheets with media="print" are appended after uncoditional
    /// style sheets,
    ///        because print-only CSS needs higher priority to override the
    ///        style used for printing.
    void startPrintMedia() { isPrintMedia = true; }

    void appendFile(QString const &fileName) {
	if (!QFileInfo(fileName).isFile()) return;
	...
	return QLatin1String(pageBackgroundColorPropertyName,
			     pageBackgroundColorPropertyNameSize);
    }

    /// @return The page background color or an empty string if
    ///         the background color could not be found in the style sheets.
    /// @warning This function parses the style sheets that have been appended
    /// to @a result. Therefore it
    ///          must be called after appending all style sheets that may
    ///          override the page background color.
    string findPageBackgroundColor(
	vector<QString> &unspecifiedColorFileNames) const {
	Q_ASSERT(needPageBackgroundColor);

	string backgroundColor;

	// Iterate in the reverse order because the CSS code lower in the page
	// overrides.
	std::find_if(
	    cssFiles.crbegin(), cssFiles.crend(),
	    [&backgroundColor,
	     &unspecifiedColorFileNames](QString const &fileName) {
		QFile cssFile(fileName);
		if (!cssFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
		    gdWarning(
			"Couldn't open CSS file \"%s\" for reading: %s (%d)",
			qUtf8Printable(fileName),
			qUtf8Printable(cssFile.errorString()),
			static_cast<int>(cssFile.error()));
		    return false;
		}
		...
		// Empty backgroundColor means that this CSS file does not
		// override page background color, in which case we look for it
		// in the remaining CSS files.
   ...
   private:
	...
	if (fileName.startsWith(QLatin1String(":/"))) {
	    // Replace the local file resource prefix ":/" with "qrc:///" for
	    // the web page.
	    result = "qrc:///";
	    result += fileName.toUtf8().constData() + 2;
	} else {
	    // fileName must be a local filesystem path => convert it into a
	    // file URL.
#ifdef Q_OS_WIN32
	    result = "file:///";
#else
...
#ifndef USE_QTWEBKIT
    static constexpr char pageBackgroundColorPropertyName[] =
	"--gd-page-background-color";
    // - 1 accounts for the terminating null character.
    static constexpr std::size_t pageBackgroundColorPropertyNameSize =
	sizeof(pageBackgroundColorPropertyName) / sizeof(char) - 1;

    vector<QString> cssFiles;

    /// Finds the page background color in @p cssFile.
    /// First looks for the page background color CSS property. If the property
    /// specification is missing, falls back to a slower search of the
    /// background color of the <body> element.
    /// @param[out] backgroundColor is set to the page background color or to an
    /// empty string
    ///             if the CSS file does not override page background color.
    /// @return true if a valid page background color CSS property specification
    /// was found, false otherwise.
    static bool findPageBackgroundColor(QFile &cssFile,
					string &backgroundColor) {
	Q_ASSERT(cssFile.isOpen());

	// TODO: remove the regex fallback, including this variable, all its
	// uses and findBodyBackgroundColor(), once users have had some time to
	// add the page background color property into their article style
	// files.
	string cssCode;

	// At the time of writing, each line in built-in article style files
	// fits into maxReasonableLineLength.
	constexpr int maxReasonableLineLength = 212;
	QVarLengthArray<char, maxReasonableLineLength> line(
	    maxReasonableLineLength);
	int offset = 0;
	...
	while (!cssFile.atEnd()) {
	    ...
	    if (bytesRead == -1) {
		gdWarning("Error while reading CSS file \"%s\": %s (%d)",
	    ...
	    Q_ASSERT(bytesRead < maxSize);  // QIODevice::readLine() reads up to
					    // a maximum of maxSize - 1 bytes.
	    if (bytesRead == maxSize - 1 && line[line.size() - 2] != '\n' &&
		!cssFile.atEnd()) {
		// This line is longer than line.size() => increase the buffer
		// size until the entire line fits in.
		Q_ASSERT(
		    line.back() ==
		    0);	 // A terminating '\0' byte is always appended to data.
		offset =
		    line.size() - 1;  // Overwrite the terminating '\0'
				      // character during the next iteration.
	    ...
	    Q_ASSERT(line[bytesRead] ==
		     0);  // A terminating '\0' byte is always appended to data.
	    // We require the property and its value to be on the same line to
	    // be able to process the CSS file line by line.
	    if (findPageBackgroundColorProperty(
		    line.data(), line.data() + bytesRead, backgroundColor))
		return true;
	    offset = 0;
	...
	gdWarning(
	    "Page background color specification is missing from CSS file "
	    "\"%s\"",
	    qUtf8Printable(cssFile.fileName()));

	backgroundColor = findBodyBackgroundColor(
	    cssCode.data(), cssCode.data() + cssCode.size());
	return false;
    }
    ...
    /// @param[out] backgroundColor is set to the page background color if the
    /// CSS property is found, unchanged otherwise.
    /// @note @p backgroundColor set to an empty string means that the CSS file
    /// does not override page background color.
    /// @return true if a valid page background color CSS property specification
    /// was found, false otherwise.
    static bool findPageBackgroundColorProperty(char const *first,
						char const *last,
						string &backgroundColor) {
	char const *it =
	    std::search(first, last, pageBackgroundColorPropertyName,
	...
	if (it == last || *it != ':') {
	    gdWarning(
		"Missing colon after %s CSS property name. Ignoring this "
    ...
    /// @return the background color or an empty string if it could not be
    /// found.
    static string findBodyBackgroundColor(char const *first, char const *last) {
	// This regular expression is simple and efficient. But the result is
	// not always accurate:
	// 1. The first word after "background:" is considered to be the color,
	// even though this first word could
	//    be something else, e.g. "border-box".
	// 2. The code inside CSS comments (/*comment*/) is matched too, not
	// skipped. Built-in and user-defined style sheets must take this
	// simplified matching into account. On the bright side, the user can
	// easily override the background color matched here by adding a comment
	// like /* body{ background:#abcdef } */ at the end of the user-defined
	// article-style.css file.
	static std::regex const backgroundRegex(
	    R"(\bbody\s*\{[^}]*\bbackground(?:|-color)\s*:\s*([^\s;}]+))",
	    // CSS code is case-insensitive => regex::icase.
	    // The regex object is reused (static) and the CSS code can be large
	    // => regex::optimize.
	    std::regex::icase | std::regex::optimize);

	// Iterate over all matches and return the last one, because the CSS
	// code lower in the page overrides.
	string::const_iterator::difference_type position = -1, length;
	for (std::cregex_iterator it(first, last, backgroundRegex), end;
	     it != end; ++it) {
	    Q_ASSERT(it->size() == 2);
...
QString propertyNameHtmlCode() {
    return wrapInHtmlCodeElement(
	CssAppender::getPageBackgroundColorPropertyName());
}

QString missingSpecificationWarningMessage(
    vector<QString> const &unspecifiedColorFileNames) {
    Q_ASSERT(!unspecifiedColorFileNames.empty());

    QString message = ArticleMaker::tr(
			  "<p>Page background color specification is missing "
			  "from the following CSS files:"
			  "</p>") +
		      QLatin1String("<pre>");
    ...
    message += ArticleMaker::tr(
	"<p>Please insert a page background color specification at the top (or "
	"close to the "
	"top) of each of these files. For example:</p>");
    // Recommend to define the page background color custom property on the
    // :root pseudo-class to allow using it globally across the CSS file in the
    // future. Do not recommend actually using the custom property value with
    // `var(--gd-page-background-color)` for now. Custom CSS properties are not
    // supported by Qt 5 WebKit, and it is important to maintain style sheet
    // compatibility with the Qt WebKit version of GoldenDict while it is widely
    // used.
    message += QLatin1String("<pre>:root\n{\n  %1: COLOR;\n}</pre>")
		   .arg(CssAppender::getPageBackgroundColorPropertyName());

    message += ArticleMaker::tr(
		   "<p>Replace %1 with the actual page background color "
		   "specified in the CSS file, that is "
		   "%2 or %3 background color. If the CSS file does not "
		   "specify the page background color, "
		   "replace %1 with an empty string (without quotes).</p>"
		   "<p>Incorrect or missing page background color "
		   "specification may cause article page "
		   "background flashes.</p>"
		   "<p>Supported page background color specification format is "
		   "strict: quotes are not "
		   "allowed, the property name %4 and its value must be on the "
		   "same line. On the other "
		   "hand, the surrounding declaration block does not matter to "
		   "GoldenDict. The property "
		   "specification can just as well be inside a CSS comment "
		   "instead of the %5 pseudo-class "
		   "block.</p>")
		   .arg(wrapInHtmlCodeElement(QLatin1String{"COLOR"}),
			bodyElementHtmlCode(), htmlElementHtmlCode(),
			propertyNameHtmlCode(),
			wrapInHtmlCodeElement(QLatin1String{":root"}));

    return message;
}

QString invalidColorWarningMessage(QString const &pageBackgroundColor) {
    return ArticleMaker::tr(
	       "<p>Invalid page background color is specified in the article "
	       "style sheets:</p>") +
	   QLatin1String("<pre>%1</pre>").arg(pageBackgroundColor) +
	   ArticleMaker::tr(
	       "<p>Set %1 to the actual page background color specified in the "
	       "CSS file, that is %2 or "
	       "%3 background color. If the CSS file does not specify the page "
	       ...
	       "href=\""
	       "https://www.w3.org/TR/SVG11/types.html#ColorKeywords\">SVG "
	       "color keyword names</a> "
	       "provided by the World Wide Web Consortium; for example, %4 or "
	       "%5.</li>"
	       "<li>%6 - representing the absence of a color.</li></ul>")
	       .arg(propertyNameHtmlCode(), bodyElementHtmlCode(),
		    htmlElementHtmlCode(),
...
ArticleMaker::ArticleMaker(
    ...
    vector<Instances::Group> const &groups_, QString const &displayStyle_,
    QString const &addonStyle_, QWidget *dialogParent_)
...
QColor ArticleMaker::colorFromString(string const &pageBackgroundColor) const {
	...
	Q_ASSERT_X(false, Q_FUNC_INFO,
		   "The default built-in style sheet :/article-style.css is "
		   "unconditionally appended "
		   "and specifies a valid page background color, which is "
		   "parsed correctly.");
	gdWarning(
	    "Couldn't find the page background color in the article style "
	    "sheets.");
	return QColor();
    }

    auto const pageBackgroundColorQString =
	QString::fromUtf8(pageBackgroundColor.c_str());
    QColor color(pageBackgroundColorQString);
    if (!color.isValid()) {
	gdWarning(
	    "Found invalid page background color in the article style sheets: "
	    "\"%s\"",
	    pageBackgroundColor.c_str());
	if (!hasShownBackgroundColorWarningMessage) {
	    hasShownBackgroundColorWarningMessage = true;
	    QMessageBox::warning(
		dialogParent, "GoldenDict",
		invalidColorWarningMessage(pageBackgroundColorQString));
	}
	return QColor();
    }

    GD_DPRINTF(
	"Found page background color in the article style sheets: \"%s\" = "
	"%s\n",
	pageBackgroundColor.c_str(),
	// Print the result of QColor's nontrivial parsing of
	// pageBackgroundColor string. Print the alpha component only if the
	// color is not fully opaque.
	qPrintable(color.name(color.alpha() == 255 ? QColor::HexRgb
						   : QColor::HexArgb)));
    return color;
}
#endif

void ArticleMaker::appendCss(string &result, bool expandOptionalParts,
	...
	cssAppender.appendFile(
	    QString(":/article-style-st-%1.css").arg(displayStyle));
    cssAppender.appendFile(Config::getUserCssFileName());
    if (!addonStyle.isEmpty())
	cssAppender.appendFile(Config::getStylesDir() + addonStyle +
	...
	auto const backgroundColorString =
	    cssAppender.findPageBackgroundColor(unspecifiedColorFileNames);
	if (!unspecifiedColorFileNames.empty() &&
	    !hasShownBackgroundColorWarningMessage) {
	    hasShownBackgroundColorWarningMessage = true;
	    QMessageBox::warning(
		dialogParent, "GoldenDict",
...
std::string ArticleMaker::makeHtmlHeader(QString const &word,
    ...
    result += "<title>" + Html::escape(Utf8::encode(gd::toWString(word))) +
	      "</title>";

    // This doesn't seem to be much of influence right now, but we'll keep
    // it anyway.
    if (icon.size())
	result += "<link rel=\"icon\" type=\"image/png\" href=\"qrc:///flags/" +
		  Html::escape(icon.toUtf8().data()) + "\" />\n";

    result +=
	"</head><body"
#ifndef USE_QTWEBKIT
	// Qt WebEngine API does not provide a way to check whether a mouse
	// click occurs on a page proper or on its scrollbar. We are only
	// interested in clicks on the page contents within <body>. Listen to
	// such mouse events and send messages from JavaScript to C++.
...
std::string ArticleMaker::makeNotFoundBody(QString const &word,
    ...
    if (word.size()) {
	gdDebug("yeah you are searching something!");
	result +=
	    tr("No translation for <b>%1</b> was found in group <b>%2</b>.")
		.arg(QString::fromUtf8(
		    Html::escape(str.toUtf8().data()).c_str()))
		.arg(QString::fromUtf8(
		    Html::escape(group.toUtf8().data()).c_str()))
		.toUtf8()
		.data();
...
sptr<Dictionary::DataRequest> ArticleMaker::makeDefinitionFor(
    ...
    QMap<QString, QString> const &contexts, QSet<QString> const &mutedDicts,
	...
	for (unsigned x = 0; x < dictionaries.size(); x++) {
	    for (QStringList::Iterator it = ids.begin(); it != ids.end();
		 ++it) {
	...
	return new ArticleRequest(phrase, "", contexts, ftsDicts, header, -1,
				  true);
    }
	...
	string result =
	    makeHtmlHeader(phrase.phrase, QString(), needExpandOptionalParts);

	if (phrase.phrase == tr("Welcome!")) {
	    result +=
		tr("<h3 align=\"center\">Welcome to <b>GoldenDict</b>!</h3>"
		   "<p>To start working with the program, first visit "
		   "<b>Edit|Dictionaries</b> to add some directory paths where "
		   "to search "
		   "for the dictionary files, set up various Wikipedia sites "
		   "or other sources, adjust dictionary order or create "
		   "dictionary groups."
		   "<p>And then you're ready to look up your words! You can do "
		   "that in this window "
		   "by using a pane to the left, or you can <a href=\"Working "
		   "with popup\">look up words from other active "
		   "applications</a>. "
		   "<p>To customize program, check out the available "
		   "preferences at <b>Edit|Preferences</b>. "
		   "All settings there have tooltips, be sure to read them if "
		   "you are in doubt about anything."
		   "<p>Should you need further help, have any questions, "
		   "suggestions or just wonder what the others think, you are "
		   "welcome at the program's <a "
		   "href=\"http://goldendict.org/forum/\">forum</a>."
		   "<p>Check program's <a "
		   "href=\"http://goldendict.org/\">website</a> for the "
		   "updates. "
		   ...
		   "or later."
		    ...
		    .toUtf8()
		    .data();
	} else if (phrase.phrase == tr("Working with popup")) {
	    result +=
		(tr("<h3 align=\"center\">Working with the popup</h3>"

		    "To look up words from other active applications, you "
		    "would need to first activate the <i>\"Scan popup "
		    "functionality\"</i> in <b>Preferences</b>, "
		    "and then enable it at any time either by triggering the "
		    "'Popup' icon above, or "
		    "by clicking the tray icon down below with your right "
		    "mouse button and choosing so in the menu you've "
		    "popped. ") +

#ifdef Q_OS_WIN32
		 tr("Then just stop the cursor over the word you want to look "
		    "up in another application, "
		    "and a window would pop up which would describe it to you.")
#else
		 tr("Then just select any word you want to look up in another "
		    "application by your mouse "
		    "(double-click it or swipe it with mouse with the button "
		    "pressed), "
		    "and a window would pop up which would describe the word "
		    "to you.")
#endif
		     )
		    .toUtf8()
...
sptr<Dictionary::DataRequest> ArticleMaker::makeNotFoundTextFor(
    ...
    string result = makeHtmlHeader(word, QString(), true) +
		    makeNotFoundBody(word, group) + "</body></html>";
...
string ArticleMaker::makeBlankPageHtmlCode(QColor *pageBackgroundColor) const {
    return makeHtmlHeader(tr("(untitled)"), QString(), true,
			  pageBackgroundColor) +
	   "</body></html>";
}
...
sptr<Dictionary::DataRequest> ArticleMaker::makePicturePage(
...
ArticleRequest::ArticleRequest(
    ...
    vector<sptr<Dictionary::Class> > const &activeDicts_, string const &header,
	...
	sptr<Dictionary::WordSearchRequest> s =
	    activeDicts[x]->findHeadwordsForSynonym(gd::toWString(word));

	connect(s.get(), SIGNAL(finished()), this, SLOT(altSearchFinished()),
		Qt::QueuedConnection);
...
void ArticleRequest::altSearchFinished() {
	...
	altsDone =
	    true;  // So any pending signals in queued mode won't mess us up
	...
	if (activeDicts.size() <= 1)
	    articleSizeLimit =
		-1;  // Don't collapse article if only one dictionary presented
...
static void appendGdMakeArticleActiveOn(string &result, char const *jsEvent,
					string const &dictionaryId) {
    result += " on";
    result += jsEvent;
...
void ArticleRequest::bodyFinished() {
	...
	if (bodyRequests.front()->isFinished()) {
		...
		if (articleSizeLimit >= 0) {
			...
			QString text = QString::fromUtf8(
			    req.getFullData().data(), req.getFullData().size());
			...
			int size = QTextDocumentFragment::fromHtml(text)
				       .toPlainText()
				       .length();
			if (size > articleSizeLimit) collapse = true;
		    } catch (...) {
		    }
		}
...
#ifdef USE_QTWEBKIT
			// gdCurrentArticleLoaded() initializes "
			// gdactivearticle" in the Qt WebEngine version.
			(closePrevSpan ? "" : " gdactivearticle") +
#endif
			(collapse ? " gdcollapsedarticle" : "") + "\" id=\"" +
			gdFrom + '"';

		// Make the article active on left, middle or right mouse button
		// click.
		appendGdMakeArticleActiveOn(head, "click", jsVal);
		// A right mouse button click triggers only "contextmenu"
		// JavaScript event.
		appendGdMakeArticleActiveOn(head, "contextmenu", jsVal);
		// In the Qt WebKit version both a left and a middle mouse
		// button click triggers "click" JavaScript event. In the Qt
		// WebEngine version a left mouse button click triggers "click",
		// a middle - "auxclick" event.
#ifndef USE_QTWEBKIT
		...
		head +=
			...
			"onclick=\"gdExpandArticle(\'") +
		    dictId + "\');" +
		    (collapse ? "\" style=\"cursor:pointer;" : "") +
		    "\" id=\"gddictname-" + Html::escape(dictId) + "\"" +
		    (collapse ? string(" title=\"") +
				    tr("Expand article").toUtf8().data() + "\""
			      : "") +
		    "><span class=\"gddicticon\"><img src=\"gico://" +
		    Html::escape(dictId) +
		    "/dicticon.png\"></span><span class=\"gdfromprefix\">" +
		    Html::escape(tr("From ").toUtf8().data()) +
		    "</span><span class=\"gddicttitle\">" +
		    Html::escape(activeDict->getName().c_str()) + "</span>" +
		    "<span class=\"collapse_expand_area\"><img "
		    "src=\"qrc:///icons/blank.png\" class=\"" +
		    (collapse ? "gdexpandicon" : "gdcollapseicon") +
		    "\" id=\"expandicon-" + Html::escape(dictId) + "\"" +
		    (collapse
			 ? ""
			 : string(" title=\"") +
			       tr("Collapse article").toUtf8().data() + "\"") +
		    "></span>" + "</div>";

		head += "<div class=\"gddictnamebodyseparator\"></div>";

		head += "<div class=\"gdarticlebody gdlangfrom-";
		head += LangCoder::intToCode2(activeDict->getLangFrom())
			    .toLatin1()
			    .data();
		head += "\" lang=\"";
		head += LangCoder::intToCode2(activeDict->getLangTo())
			    .toLatin1()
			    .data();
		head += "\"";
		head += " style=\"display:";
		head += collapse ? "none" : "inline";
		head += string("\" id=\"gdarticlefrom-") +
			Html::escape(dictId) + "\">";

		if (errorString.size()) {
		    head += "<div class=\"gderrordesc\">" +
			    Html::escape(tr("Query error: %1")
					     .arg(errorString)
					     .toUtf8()
					     .data()) +
			    "</div>";
		...
		string const articleEnding =
		    "<script>gdArticleLoaded(\"" + gdFrom + "\");</script>";

		data.resize(data.size() + head.size() +
			    (req.dataSize() > 0 ? req.dataSize() : 0) +
			    articleEnding.size());
		...
		try {
		    if (req.dataSize() > 0)
			bodyRequests.front()->getDataSlice(
			    0, req.dataSize(),
		...
		std::copy(articleEnding.begin(), articleEnding.end(),
			  data.end() - articleEnding.size());
...

	{
		...
		// Larger words are usually whole sentences - don't clutter the
		// output with their full bodies.
		footer += ArticleMaker::makeNotFoundBody(
		    word.size() < 40 ? word : "", group);

		// When there were no definitions, we run stemmed search.
		stemmedWordFinder = new WordFinder(this);

		connect(stemmedWordFinder.get(), SIGNAL(finished()), this,
			SLOT(stemmedSearchFinished()), Qt::QueuedConnection);
...
void ArticleRequest::stemmedSearchFinished() {
    ...
    if (sr.size()) {
	footer +=
	    "<div class=\"gdstemmedsuggestion\"><span "
	    "class=\"gdstemmedsuggestion_head\">" +
    ...
    {
	disconnect(stemmedWordFinder.get(), SIGNAL(finished()), this,
		   SLOT(stemmedSearchFinished()));

	connect(stemmedWordFinder.get(), SIGNAL(finished()), this,
		SLOT(individualWordFinished()), Qt::QueuedConnection);
    ...
    if (!continueMatching) footer += "</body></html>";
...
void ArticleRequest::compoundSearchNextStep(bool lastSearchSucceeded) {
	...
	if (currentSplittedWordStart >= splittedWords.first.size() - 2) {
	    ...
	    if (firstCompoundWasFound) footer += "</span>";

	    // Now add links to all the individual words. They conclude the
	    // result.
	...
	if (currentSplittedWordEnd < splittedWords.first.size() - 1) {
	    ...
	    ++currentSplittedWordEnd;  // So we could use the same code for
				       // result emitting
    ...
    stemmedWordFinder->expressionMatch(
	currentSplittedWordCompound, activeDicts,
...
void ArticleRequest::individualWordFinished() {
    ...
    if (results.size()) {
	wstring source = Folding::applySimpleCaseOnly(
	    gd::toWString(currentSplittedWordCompound));
	...
	for (unsigned x = 0; x < results.size(); ++x) {
	    ...
	    wstring result(
		Folding::applySimpleCaseOnly(gd::toWString(results[x].first)));

	    if (source.size() <= result.size() &&
		result.compare(0, source.size(), source) == 0) {
...
QPair<ArticleRequest::Words, ArticleRequest::Spacings>
ArticleRequest::splitIntoWords(QString const &input) {
	...
	for (; ptr->unicode() && (Folding::isPunct(ptr->unicode()) ||
				  Folding::isWhitespace(ptr->unicode()));
	     ++ptr)
	    spacing.append(*ptr);
...
string ArticleRequest::linkWord(QString const &str) {
    ...
    return string("<a href=\"") + url.toEncoded().data() + "\">" +
	   escapedResult + "</a>";
}
