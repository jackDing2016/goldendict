...
bool ftsIndexIsOldOrBad(string const &indexFile,
    ...
    return idx.readRecords(&header, sizeof(header), 1) != 1 ||
	   ...
	   header.formatVersion !=
	       CurrentFtsFormatVersion + dict->getFtsIndexVersion();
}

static QString makeHiliteRegExpString(QStringList const &words, int searchMode,
...
bool parseSearchString(QString const &str, QStringList &indexWords,
		       QStringList &searchWords, QRegExp &searchRegExp,
		       int searchMode, bool matchCase, int distanceBetweenWords,
		       bool &hasCJK) {
    searchWords.clear();
    indexWords.clear();
    ...
    QRegExp regexRegExp(
	"\\\\[afnrtvdDwWsSbB]|\\\\x([0-9A-Fa-f]{4})|\\\\0([0-7]{3})",
	...
	QString searchString =
	    makeHiliteRegExpString(allWords, searchMode, distanceBetweenWords);

	searchRegExp = QRegExp(
	    searchString, matchCase ? Qt::CaseSensitive : Qt::CaseInsensitive,
	    QRegExp::RegExp2);
	...
	if (searchMode == FTS::RegExp) tmp.replace(regexRegExp, " ");
	...
	if (hasCJK) {
		...
		if (!parsed) wordList.append(word);
	    }
	...
	searchRegExp =
	    QRegExp(str, matchCase ? Qt::CaseSensitive : Qt::CaseInsensitive,
		    searchMode == FTS::Wildcards ? QRegExp::WildcardUnix
						 : QRegExp::RegExp2);
	searchRegExp.setMinimal(true);
    }
...
void parseArticleForFts(uint32_t articleAddress, QString &articleText,
    ...
    if (articleText.isEmpty()) return;

#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
    QRegularExpression regBrackets(
	"(\\([\\w\\p{M}]+\\)){0,1}([\\w\\p{M}]+)(\\([\\w\\p{M}]+\\)){0,1}(["
	"\\w\\p{M}]+){0,1}(\\([\\w\\p{M}]+\\)){0,1}",
	QRegularExpression::UseUnicodePropertiesOption);
    QRegularExpression regSplit("[^\\w\\p{M}]+",
				QRegularExpression::UseUnicodePropertiesOption);

    QStringList articleWords =
	articleText.normalized(QString::NormalizationForm_C)
	    .split(QRegularExpression(
		       handleRoundBrackets ? "[^\\w\\(\\)\\p{M}]+"
					   : "[^\\w\\p{M}]+",
		       QRegularExpression::UseUnicodePropertiesOption),
		   Qt4x5::skipEmptyParts());
#else
    QRegExp regBrackets = QRegExp(
	"(\\(\\w+\\)){0,1}(\\w+)(\\(\\w+\\)){0,1}(\\w+){0,1}(\\(\\w+\\)){0,1}");
    QRegExp regSplit = QRegExp("\\W+");

    QStringList articleWords =
	articleText.normalized(QString::NormalizationForm_C)
	    .split(QRegExp(handleRoundBrackets ? "[^\\w\\(\\)]+" : "\\W+"),
		   Qt4x5::skipEmptyParts());
#endif
	...
	// If word contains CJK symbols we add to index only these symbols
	// separately
	...
	if (!hasCJK) {
	    ...
	    if (word.size() < FTS::MinimumWordSize) continue;

	    if (handleRoundBrackets &&
		(word.indexOf('(') >= 0 || word.indexOf(')') >= 0)) {
		// Special handle for words with round brackets - DSL feature
		QStringList list;

		QStringList oldVariant =
		    word.split(regSplit, Qt4x5::skipEmptyParts());
		for (QStringList::iterator it = oldVariant.begin();
		     it != oldVariant.end(); ++it)
		    if (it->size() >= FTS::MinimumWordSize &&
			!list.contains(*it))
			list.append(*it);
		...
		if (match.hasMatch()) {
		    ...
		    // Add empty strings for compatibility with QRegExp
		    // behaviour
...
#endif
		    QString parsedWord =
			parts[2] + parts[4];  // Brackets removed

		    if (parsedWord.size() >= FTS::MinimumWordSize &&
			!list.contains(parsedWord))
			list.append(parsedWord);

		    parsedWord =
			parts[1].remove('(').remove(')') + parts[2] +
			parts[3].remove('(').remove(')') + parts[4] +
	    ...
	    } else if (!setOfWords.contains(word)) {
...
void makeFTSIndex(BtreeIndexing::BtreeDictionary *dict,
		  QAtomicInt &isCancelled) {
    Mutex::Lock _(dict->getFtsMutex());

    if (Qt4x5::AtomicInt::loadAcquire(isCancelled)) throw exUserAbort();
	...
	*ptr = *it;
    ...
    {
	QString name =
	    QString::fromUtf8(dict->getDictionaryFilenames()[0].c_str())
		.toLower();
	...
	out << "<" << headword << ">" << " " << articleStr << "<end>\n";

	parseArticleForFts(offsets.at(i), articleStr, ftsWords,
			   needHandleBrackets);
	gdDebug("finish todo something ");
    }
...
#define BUF_SIZE 20000
	...
	wordsWithOffsets.append(
	    QPair<gd::wstring, uint32_t>(gd::toWString(it.key()), offset));
	...
	if (wordsWithOffsets.size() >= BUF_SIZE) {
		...
		if (Qt4x5::AtomicInt::loadAcquire(isCancelled))
		    throw exUserAbort();
		indexedWords.addSingleWord(wordsWithOffsets[i].first,
					   wordsWithOffsets[i].second);
	    }
...
#undef BUF_SIZE
    ...
    BtreeIndexing::IndexInfo ftsIdxInfo =
	BtreeIndexing::buildIndex(indexedWords, ftsIdx);
    ...
    ftsIdxHeader.formatVersion =
	FtsHelpers::CurrentFtsFormatVersion + dict->getFtsIndexVersion();
...
void FTSResultsRequestRunnable::run() { r.run(); }
...
void FTSResultsRequest::checkArticles(QVector<uint32_t> const &offsets,
	...
	if (searchMode == FTS::Wildcards)
		...
		wildcardsToRegexp(searchRegexp.pattern()));
	else
...
#endif
	for (int i = 0; i < offsets.size(); i++) {
	    if (Qt4x5::AtomicInt::loadAcquire(isCancelled)) break;

	    dict.getArticleText(offsets.at(i), headword, articleText);
	    articleText = articleText.normalized(QString::NormalizationForm_C);

	    if (ignoreDiacritics)
		articleText = gd::toQString(
		    Folding::applyDiacriticsOnly(gd::toWString(articleText)));
		...
		if (headword.isEmpty())
		    ...
		    foundHeadwords->append(FTS::FtsHeadword(
			headword, id, QStringList(), matchCase));

		results++;
		if (maxResults > 0 && results >= maxResults) break;
	    }
	}
    } else {
	// Words mode
...
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
	QRegularExpression splitWithBrackets(
	    "[^\\w\\(\\)\\p{M}]+",
	    QRegularExpression::UseUnicodePropertiesOption);
	QRegularExpression splitWithoutBrackets(
	    "[^\\w\\p{M}]+", QRegularExpression::UseUnicodePropertiesOption);
#else
	...
	if (ignoreWordsOrder) {
	    for (QStringList::const_iterator it = words.begin();
		 it != words.end(); ++it)
	...
	for (int i = 0; i < offsets.size(); i++) {
	    ...
	    QStringList articleWords = articleText.split(
		needHandleBrackets ? splitWithBrackets : splitWithoutBrackets,
		Qt4x5::skipEmptyParts());
		...
		if (needHandleBrackets &&
		    ...
		    // Handle brackets
#if QT_VERSION >= QT_VERSION_CHECK(5, 0, 0)
		    QRegularExpressionMatch match_brackets =
			regBrackets.match(s);
		    if (match_brackets.hasMatch()) {
			QStringList parts = match_brackets.capturedTexts();
			// Add empty strings for compatibility with QRegExp
			// behaviour
			for (int i = match_brackets.lastCapturedIndex() + 1;
			     i < 6; i++)
			    parts.append(QString());
#else
			...
			word = parts[1].remove('(').remove(')') + parts[2] +
			       parts[3].remove('(').remove(')') + parts[4] +
			       parts[5].remove('(').remove(
				   ')');  // Brackets expansed
			parsedWords.append(word);
		    } else
			parsedWords =
			    s.split(regSplit, Qt4x5::skipEmptyParts());
		} else
		...
		for (n = 0; n < parsedWords.size(); n++) {
			...
			for (i = 0; i < wordsList.size(); i++) {
				...
				if ((searchMode == FTS::WholeWords &&
				     parsedWords.at(n).compare(
					 wordsList.at(i).first, cs) == 0) ||
				    (searchMode == FTS::PlainText &&
				     parsedWords.at(n).contains(
					 wordsList.at(i).first, cs))) {
				    wordsList[i].second = false;

				    if (parsedWords.size() > 1) {
					QString wordToHilite = s;
					while (!wordToHilite.isEmpty() &&
					       (wordToHilite.at(0) == '(' ||
						wordToHilite.at(0) == ')'))
					    wordToHilite.remove(0, 1);
					while (!wordToHilite.isEmpty() &&
					       (wordToHilite.endsWith('(') ||
						wordToHilite.endsWith(')')))
					    wordToHilite.chop(1);
					order.append(
					    wordToHilite.replace('(', "\\(")
						.replace(')', "\\)"));
				    } else
			...
			if (i < wordsList.size()) {
			    ...
			    if (matchWordNom == 1) {
				// Store position to remake search if sequence
				// will not be found
				...
				// All words are found
				// Store found words sequence and continue
				// search It's nesessary for hilite search
				// results

				// Check if such sequence already presented
				int x;
				for (x = 0; x < allOrders.size(); x++) {
				    if (allOrders[x] == order) break;
				}
				if (x >= allOrders.size())
			    ...
			    unmatchWordNom = 0;
			    break;
			} else if (matchWordNom > 0 &&
				   n >= parsedWords.size() - 1) {
			    unmatchWordNom += 1;
			    if (distanceBetweenWords >= 0 &&
				unmatchWordNom > distanceBetweenWords) {
			...
			if ((searchMode == FTS::WholeWords &&
			     parsedWords.at(n).compare(words.at(matchWordNom),
						       cs) == 0) ||
			    (searchMode == FTS::PlainText &&
			     parsedWords.at(n).contains(words.at(matchWordNom),
							cs))) {
			    matchWordNom += 1;
				...
				matchWordNom = 0;
				unmatchWordNom = 0;
				if (needHandleBrackets) order.clear();
			    }
			}
		    }
		}
		...
		if (!allOrders.isEmpty()) {
			...
			QString hiliteStr = makeHiliteRegExpString(
			    allOrders.at(i), searchMode, distanceBetweenWords);
		...
		if (headword.isEmpty()) {
		    ...
		    foundHeadwords->append(
			FTS::FtsHeadword(headword, id, hiliteReg, matchCase));
...
void FTSResultsRequest::indexSearch(BtreeIndexing::BtreeIndex &ftsIndex,
    ...
    if (indexWords.isEmpty()) return;

    int n = indexWords.length();
    for (int i = 0; i < n; i++) {
	if (Qt4x5::AtomicInt::loadAcquire(isCancelled)) return;
    ...
    if (setOfOffsets.isEmpty()) return;
...
void FTSResultsRequest::combinedIndexSearch(BtreeIndexing::BtreeIndex &ftsIndex,
    ...
    if (!hieroglyphsList.isEmpty()) n += 1;
	...
	for (int x = 0; x < links.size(); x++) {
	    ...
	    QString word =
		QString::fromUtf8(links[x].word.data(), links[x].word.size());

	    if (ignoreDiacritics)
		word = gd::toQString(
		    Folding::applyDiacriticsOnly(gd::toWString(word)));

	    for (int i = 0; i < wordsList.size(); i++) {
		if (word.length() >= wordsList.at(i).length() &&
		    word.contains(wordsList.at(i))) {
		    vector<char> chunk;
		    char *linksPtr;
		    {
			Mutex::Lock _(dict.getFtsMutex());
			linksPtr =
			    chunks->getBlock(links[x].articleOffset, chunk);
		    }

		    memcpy(&size, linksPtr, sizeof(uint32_t));
		    linksPtr += sizeof(uint32_t);
		    for (uint32_t y = 0; y < size; y++) {
			allWordsLinks[wordNom].insert(
			    *(reinterpret_cast<uint32_t *>(linksPtr)));
			linksPtr += sizeof(uint32_t);
		    }
		    wordNom += 1;
		    if (searchMode == FTS::PlainText ||
			searchMode == FTS::WholeWords)
			break;
		}
	    }
	}
...
void FTSResultsRequest::fullIndexSearch(BtreeIndexing::BtreeIndex &ftsIndex,
	...
	for (int i = 0; i < indexWords.size(); i++) {
	    if (word.length() >= indexWords.at(i).length() &&
		word.contains(indexWords.at(i))) {
		vector<char> chunk;
		char *linksPtr;
		{
		    Mutex::Lock _(dict.getFtsMutex());
		...
		linksPtr += sizeof(uint32_t);
		for (uint32_t y = 0; y < size; y++) {
		    allWordsLinks[i].insert(
			*(reinterpret_cast<uint32_t *>(linksPtr)));
		    linksPtr += sizeof(uint32_t);
		}
		if (searchMode == FTS::PlainText) break;
	    }
	}
    }
...
void FTSResultsRequest::run() {
	...
	if (!FtsHelpers::parseSearchString(
		searchString, indexWords, searchWords, searchRegExp, searchMode,
		matchCase, distanceBetweenWords, hasCJK)) {
	    finish();
	    return;
	}
	...
	if (dict.haveFTSIndex() && !indexWords.isEmpty()) {
		...
		ftsIndex.openIndex(
		    BtreeIndexing::IndexInfo(ftsIdxHeader.indexBtreeMaxElements,
					     ftsIdxHeader.indexRootOffset),
		    ftsIdx, dict.getFtsMutex());

		chunks = new ChunkedStorage::Reader(ftsIdx,
						    ftsIdxHeader.chunksOffset);
	    }

	    if (hasCJK)
		combinedIndexSearch(ftsIndex, chunks, indexWords, searchWords,
				    searchRegExp);
	    else {
		if (searchMode == FTS::WholeWords)
		    indexSearch(ftsIndex, chunks, indexWords, searchWords);
		else
		    fullIndexSearch(ftsIndex, chunks, indexWords, searchWords,
				    searchRegExp);
	    }
	} else {
